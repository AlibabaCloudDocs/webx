...
|配置项|配置名|默认值|值域|描述|
|---|---|---|--|--|
|instanceTypeinstanceType|实例规格|\["ecs.g5.large"\]| |部署环境使用的实例规格，Web+将使用所选择的规格创建ECS实例。可选择多个规格，当所选规格库存不足时，Web+将依据选择的顺序尝试创建，最多选择10种实例规格。|
...
|internetMaxBandwidthOut|公网出带宽最大值|50|\[0,100\]|，单位为 Mbps \(Mega bit per second\)，取值范围：按带宽计费：0~100，如果您没有指定该参数，则出带宽将自动被设置为 0 Mbps。按流量计费：0~100，如果您没有指定该参数，则会出现报错。|
